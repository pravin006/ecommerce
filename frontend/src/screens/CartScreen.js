    ...
    return (
        <Row>
            <Col md={8}>
                <h1>Shopping Cart</h1>
                {cartItems.length === 0?(
                    <Message variant='info'>Your cart is empty<Link to ='/' className='btn'>Go Back</Link></Message>
                ): (
                        ...
                        {cartItems.map(item => (
                            <ListGroup.Item key={item.product}>
                                <Row>
                                    ...
                                    <Col md={3}>
                                        <Link to={`/product/${item.product}`} style={{ textDecoration: 'none', color: 'inherit' }}>{item.name}</Link>
                                    </Col>
